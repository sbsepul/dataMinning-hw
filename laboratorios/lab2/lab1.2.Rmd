...
#dim(data_tf)[0] #numero de columnas
#dim(data_tf)[1] #numero de filas
```
...
#pat <- data_tf[order(data_tf$patriotismo, c("localidades")), ]
...
ggplot(a1) +
  geom_bar(aes(x = concepto, y = valor), stat="identity") +   
  coord_flip() +  
...
#a1 %>%
#  filter(region=="Atacama") %>%
#  arrange(region, -valor)
  
#a2 <- aggregate(valor ~ region , a, FUN=max)
#combi <- merge(a1,a2)

#v %>%
#  group_by(region,valores)
```
